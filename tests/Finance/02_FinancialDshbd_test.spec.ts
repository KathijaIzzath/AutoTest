...
async function openClientSearch(page: Page): Promise<void> {
  ...
  const searchButton = page.getByRole('button', { name: d.buttons.search });
  if (await searchButton.isVisible().catch(() => false)) return;

  const activeSiteInput = page.locator(d.selectors.appClientSearch).getByRole('textbox');
  if (await activeSiteInput.isVisible().catch(() => false)) {
    await activeSiteInput.click();
    const siteHint = d.searchValues.clientDropdownOption.split(' ').pop() || d.searchValues.clientDropdownOption;
    await activeSiteInput.fill(siteHint);
    const optionsList = page.getByRole('listbox', { name: 'Options list' });
    await expect(optionsList).toBeVisible({ timeout: d.timeouts.generalTimeout });

    const preferredOption = optionsList.getByRole('option').filter({ hasText: d.searchValues.clientDropdownOption }).first();
    if ((await preferredOption.count()) > 0) {
      await preferredOption.click();
    } else {
      const firstOption = optionsList.getByRole('option').first();
    ...
    const transactionType = page.getByText(d.labels.transactionType).locator('xpath=following::select[1]').first();
    const transactionTypeEnabled = await transactionType.isEnabled().catch(() => false);
    if (!transactionTypeEnabled && !(await searchButton.isVisible().catch(() => false))) {
      await activeSiteInput.press('Enter');
    }
  }
  ...
  if (await byPatientHeading.isVisible().catch(() => false)) return;

  const firstGridCell = page.locator('tbody tr').first().locator('td').first();
  if ((await firstGridCell.count()) > 0) return;

  await expect(activeSiteInput).toBeVisible({ timeout: d.timeouts.searchTimeout });
}
...
async function selectPatientRowIfPresent(page: Page): Promise<void> {
  ...
  if (await patientCell.first().isVisible().catch(() => false)) {
...
test('Process Payments - search by first name returns results table', async ({ page }) => {
  ...
  await fillNameSearchAndSubmit(page, d.searchValues.firstNameSearch);
...
test('Financial dashboard - process payments, add wallet method, and verify responsible party search', async ({ page }) => {
  ...
  await expect(page.getByRole('columnheader', { name: d.headers.id })).toBeVisible();
  await expect(page.getByRole('columnheader', { name: d.headers.name })).toBeVisible();
  await expect(page.getByRole('cell', { name: d.searchValues.firstSearchResultId })).toBeVisible();
  await expect(page.getByRole('cell', { name: d.searchValues.firstSearchResultName })).toBeVisible();
  await expect(page.getByRole('link', { name: d.selectors.viewAccountBalance }).first()).toBeVisible();
  await expect(page.getByTitle(d.selectors.addToWallet).first()).toBeVisible();
  ...
  await expect(page.getByRole('cell', { name: userData.financial.patientNameforSearch })).toBeVisible();
  await expect(page.getByRole('heading', { name: d.labels.addPaymentMethod })).toBeVisible();
  await expect(page.getByText(d.labels.select)).toBeVisible();
  ...
  await expect(page.getByRole('cell', { name: userData.financial.onlyIdentifier })).toBeVisible();
  await expect(page.getByRole('cell', { name: userData.financial.patientNameforSearch })).toBeVisible();
  await expect(page.getByRole('link', { name: d.selectors.viewAccountBalance })).toBeVisible();
  await expect(page.getByTitle(d.selectors.addToWallet)).toBeVisible();
  ...
  await page.getByRole('button', { name: d.buttons.search }).click();
  await expect(page.getByRole('heading', { name: `${d.labels.chargeSummaryColonPrefix} ${userData.financial.partyID}` })).toBeVisible();
  await expect(page.getByRole('heading', { name: d.labels.responsiblePartyName })).toBeVisible();
  await expect(page.getByRole('cell', { name: userData.financial.onlyIdentifier })).toBeVisible();
  await expect(page.getByRole('cell', { name: userData.financial.patientNameforSearch })).toBeVisible();
  await expect(page.getByRole('cell', { name: d.selectors.viewAccountBalance })).toBeVisible();
...
test('Client search page - By Patient Name and By Responsible Party sections are visible', async ({ page }) => {
  ...
  const byPatientHeading = page.getByRole('heading', { name: d.labels.byPatientName });
  const byResponsibleHeading = page.getByRole('heading', { name: d.labels.byResponsibleParty });
  if (await byPatientHeading.isVisible().catch(() => false)) {
    await verifyElementsVisible([
      byPatientHeading,
      byResponsibleHeading,
...
test('Client search by Identifier returns Charge Summary heading and patient row', async ({ page }) => {
  ...
  const chargeHeading = page.getByRole('heading', { name: `${d.labels.chargeSummaryColonPrefix} ${userData.financial.partyID}` });
  const found = await chargeHeading.isVisible({ timeout: d.timeouts.searchTimeout }).catch(() => false);
  test.skip(!found, 'Charge Summary for configured patient identifier is not available in QA  skipping.');
  if (!found) return;
...
test('Client search with invalid Identifier returns no Charge Summary', async ({ page }) => {
  ...
  const chargeHeadingVisible = await page
    .getByRole('heading', { name: new RegExp(d.labels.chargeSummaryColonPrefix, 'i') })
    .first()
    .isVisible({ timeout: Math.min(d.timeouts.searchTimeout ?? 15000, 10000) })
    .catch(() => false);
  expect(chargeHeadingVisible, 'Invalid identifier must not produce a Charge Summary heading').toBeFalsy();
});
